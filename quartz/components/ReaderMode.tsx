...
const ReaderMode: QuartzComponent = ({ displayClass, cfg }: QuartzComponentProps) => {
  const title = i18n(cfg.locale).components.readerMode.title
  return (
    <button
      type="button"
      ...
      aria-label={title}
      title={title}
      data-i18n-aria-label-key="components.readerMode.title"
      data-i18n-title-key="components.readerMode.title"
    >
      <svg
        ...
        aria-hidden="true"
      >
        <title data-i18n-key="components.readerMode.title">{title}</title>
